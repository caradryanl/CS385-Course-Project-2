...
def main():    
    ...
    solvers = []
    for i in range(9, 19):
        solvers.append(GradCam(model, model.model, str(i)))
    ...
    for cls_num, cls_path in enumerate(os.listdir(args.inputdir)):
        ...
        for img_id, img_path in enumerate(os.listdir(cls_path_abs)):
            ...
            x = x.unsqueeze(0)
            for i in range(0, 10):
                gray_cam = solvers[i](x)
                heatmap = solvers[i].heatmap(img, gray_cam)

                # save the heatmap
                name = 'GradCam_' + cls_path +  '_' + str(i) + '.jpg'
                heatmap = np.clip(heatmap, 0, 1)
                heatmap = np.uint8(heatmap * 255)
                big_heatmap[cls_num*32:(cls_num+1)*32, i*32:(i+1)*32, :] = heatmap

            print(heatmap.shape)
            break
            # cv2.imwrite(os.path.join(args.outputdir, cls_path, name), heatmap)
    cv2.imwrite('./heatmap_seq.jpg', big_heatmap)
    cv2.imwrite('./img_seq.jpg', big_img)
